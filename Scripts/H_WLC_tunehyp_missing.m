function Dat = H_WLC_tunehyp_missing(Arc);
...
%What are the observed loci/missing loci?
obs{1,1} = 1:1:10; 
obs{1,2} = 20:1:100;
...
Delta_1      = sqrt(diff(Xtrue(j,obs{j,1})).^2 + diff(Ytrue(j,obs{j,1})).^2 + diff(Ztrue(j,obs{j,1})).^2);
Delta_2      = sqrt(diff(Xtrue(j,obs{j,2})).^2 + diff(Ytrue(j,obs{j,2})).^2 + diff(Ztrue(j,obs{j,2})).^2);

E1_1         = [(diff(Xtrue(j,obs{j,1}))./Delta_1)',(diff(Ytrue(j,obs{j,1}))./Delta_1)',(diff(Ztrue(j,obs{j,1}))./Delta_1)'];
E1_2         = [(diff(Xtrue(j,obs{j,2}))./Delta_2)',(diff(Ytrue(j,obs{j,2}))./Delta_2)',(diff(Ztrue(j,obs{j,2}))./Delta_2)'];

A1 = diag(E1_1*E1_1',1);
A2 = diag(E1_2*E1_2',1);

En_1         = sum(k(1:length(A1))'.*A1);
En_2         = sum(k(end-length(A2)+1:end)'.*A2);


%Estimate partition function
Parameters = [No,len,Radius,U,a,k(1:length(A1))];  %Group parameters.
Zan1(j,1)   = EstimatePartition(Parameters);
Parameters = [No,len,Radius,U,a,k(end-length(A2)+1:end)];  %Group parameters.
Zan1(j,2)   = EstimatePartition(Parameters);

PolyLA(j,1) = (1/a)*En_1 + (1/a)*En_2;


end

Zan(1,1)   = sum(sum(Zan1));%EstimatePartition(Parameters);
...
Po(1,1) = PolyL(1,1) + GPL(1,1) + Phyp(1,1) - Zan(1,1);%*length(PInd)
...
%En_1         = sum(k(obs{j,1})'.*diag(E1_1*E1_1',1));
%En_2         = sum(k(obs{j,2})'.*diag(E1_2*E1_2',1));
...
PolyL(i,1) = sum(PolyLA);
    ...
    Po(i,1) = PolyL(i,1) + GPL(i,1) + Phyp(i,1) - Zan(i,1);%*length(PInd);
...
mkdir('./HWLC_r')
save(['./HWLC_r/InCorrectPrior_Tunep_Tunehyp_' num2str(jki) '_missing.mat'],'Dat')
