function Dat = H_WLC_tunehyp_em(Arc);
...
%load('HWLC_r/EmulatePartition_1B.mat')
...
%hyp2.lik = log(1);
%hyp2.mean = ones(1,99)';
%hyp2.mean = mean(Part.Zantrue(1:1000,1));
%Optimise hyperparams
%hyp2 = minimize(hyp2, @gp, -1000, @infExact, meanfunc, @covSEiso, @likGauss,  Xt(1:1000,:),  Yp(1:1000,1));
load EmulateParams.mat
...
No2      = 100000;                 %Number steps in Marko Chain
...
%keyboard
[Zan1(1,1) s2 m2 s3] = gp(EmulatParams.hyp2, @infExact, EmulatParams.meanfunc2, @covSEiso, @likGauss, EmulatParams.Xall(1:1100,:),  EmulatParams.Yall(1:1100)/1000, sort(Y(1,:),2)-1);

Zan(1,1) = Zan1(1,1)*1000;
...
for i = 2:No2
    ...
    [Zan1(1,1) s2 m2 s3] = gp(EmulatParams.hyp2, @infExact, EmulatParams.meanfunc2, @covSEiso, @likGauss, EmulatParams.Xall(1:1100,:),  EmulatParams.Yall(1:1100)/1000, sort(Y(i,:),2)-1);
...
Partitiontrue{nochains} = Zantrue;
...
mkdir('./HWLC_r')
save(['./HWLC_r/InCorrectPrior_Tunep_Tunehyp_' num2str(jki) '_simdata_em2.mat'],'Dat')
