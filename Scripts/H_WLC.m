...
Po(1,1) = PolyL(1,1) + GPL(1,1) - indvals(1,jki)*Zan(1,1);%*length(PInd)
...
PolyL(i,1) = sum(PolyLA);
    ...
    Po(i,1) = PolyL(i,1) + GPL(i,1) - indvals(1,jki)*Zan(i,1);%*length(PInd);
...
%mkdir('./HWLC_r')
%save(['./HWLC_r/InCorrectPrior_Tunep_' num2str(jki) '.mat'],'Dat')
