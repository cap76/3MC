function [X,Y,Z,Energy_WLC] = BouquetPairing(varargin);
%
%Description
%
...
thet            = ones(No,1)*angle;%rand(No,1)*2*pi;
...
Bond4 = info.Bond4;
lenB = info.lenB;
%ifo.Xrot = [xxx,yxx,zxx];
%info.uu      = [xxx',yxx',zxx'];
%info.Bond3   = randint2(No2,1,2)*(len-1) +1;
%info.Bond4   = randint2(No2,1,len-2)+2;
%info.lenB    = randint2(No2,1,len-2)+2;
xxx = Xrot(:,1);
yyy = Xrot(:,2);
zzz = Xrot(:,3);
...
subsample       = 1:1:No;%linspace(50000,100000,5001);
X = X(subsample,:); 
Y = Y(subsample,:); 
Z = Z(subsample,:); 
