function [net] = conditionalBouquet2Rabl(varargin);

%Rabl2Bouquet - Markov chain Monte Carlo algorithm for simulating chromosome trajectories when telomeres and centromeres are attached to the nuclear periphery and conditionally distributed according to a von Mises-Fisher distribution. 
...
%
% usage: [X,Y,Z,Energy] = Rabl2Bouquet(Parameters,x,y,z,VMF) 
%
% where
%
...
case 2
	if length(varargin{1})==6 && size(varargin{2})==[3,2]
		Parameters = varargin{1};
		VMF = varargin{2}
        [x0,y0,z0,theta,phi]    = tMultivMF(0,0,1,Parameters(1,3),Parameters(1,4),1);
        [x01,y01,z01,theta,phi] = tMultivMF(0,0,1,Parameters(1,3),Parameters(1,4),1);
