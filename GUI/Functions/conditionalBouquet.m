function [net] = conditionalBouquet(varargin);

%Bouquet - Markov chain Monte Carlo algorithm for simulating chromosome trajectories when telomeres are attached to the nuclear periphery and conditionally distributed according to a von Mises-Fisher distribution. 
%Three types of update are defined in the MCMC search: (i) A crankshaft
%rotation about an axis connecting two randomly determined bonds; (ii) A crankshaft rotation of one arm; and (iii) Rotation of chromosomes via random axis. 
%
% usage: [X,Y,Z,Energy] = BOUQUET(Parameters,x,y,z,VMF) 
%
% where
%
...
%
% PEB added lots of these calls 
%
% if UpdateProgress(message) return;
%
% This is a subfunction added to the bottom of this file. Its purpose is to
% provide feedback to the GUI. It prints the message, increments the
% progress bar, and check to see if the user has cancelled the analysis. If
% so, Bouquet will return an empty structure;
%
%
%
