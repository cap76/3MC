function [net] = FreeWLC(varargin);

%FreeWLC - Markov chain Monte Carlo algorithm for simulating chromosome
%trajectories when chromosomes are confined to the nuclear periphery but
%otherwise untethered.
%Three types of update are defined in the MCMC search: (i) A crankshaft
%rotation about an axis connecting two randomly determined bonds; (ii) A crankshaft rotation of either arm; and (iii) Rotation of chromosomes via a random axis. 
%
% usage: [X,Y,Z,Energy] = FREEWLC(Parameters,x,y,z,VMF) 
%
% where
%
...
%
% Example use can be found by running the script demFreeWLC.m
%
